...
# Kustomize overlay used to render the manager deployment.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name azure-workload-identity-operator-builder
	$(CONTAINER_TOOL) buildx use azure-workload-identity-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm azure-workload-identity-operator-builder
	rm Dockerfile.cross
...
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment.
	...
	"$(KUSTOMIZE)" build "$(DEPLOY_CONFIG)" > dist/install.yaml
