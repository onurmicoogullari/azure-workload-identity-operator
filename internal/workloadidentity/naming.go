...
func ValidateUserAssignedIdentityName(name string) error {
	if len(name) < 3 || len(name) > maxUserAssignedIdentityNameLength {
		return fmt.Errorf("user assigned identity name must contain between 3 and %d characters", maxUserAssignedIdentityNameLength)
	}
	if !userAssignedIdentityNamePattern.MatchString(name) {
		return fmt.Errorf("user assigned identity name must contain only alphanumeric characters, hyphens, and underscores and must start with an alphanumeric character")
	}
	return nil
}
